...
def load_corpus(sample_rate, layer_names, pcts, thresholds, thresholdPcts):
    print('Load corpus')
    ...
    for i in layer_names:
        acts_pieces_by_layer.append([])
        img_pieces_by_layer.append([])

    for img in imgs:
        acts_by_layer = get_layers_output(layers, layer_names, img)

        for i, acts in enumerate(acts_by_layer):
            meta_idx = int(layer_names[i][4]) - 1
            acts_pieces, img_pieces, locations = get_pieces_for_layer(img, acts, layers_meta[meta_idx], pcts[i], thresholds[i], thresholdPcts[i])
    ...
    for i in range(len(layer_names)):
        print('For L' + str(layer_names[i]) + ' loaded ' + str(len(acts_pieces_by_layer[i])) + ' acts and ' + str(len(img_pieces_by_layer[i])) + ' imgs')
